...
function fixar_site_menu($class = null) {
...
function fixar_show_breadcrumbs(){
    if ( class_exists( 'WooCommerce' ) && !is_page_template( 'page-home.php' )) {
        woocommerce_breadcrumb();
    } elseif ( function_exists( 'fixar_breadcrumbs' ) && !is_page_template( 'page-home.php' ) ){
        fixar_breadcrumbs();
    }
}

// numbered pagination
function fixar_num_pagination( $pages = '', $range = 2 ) {
	 $showitems = ( $range * 2 ) + 1;
	 ...
	 if ( 1 != $pages )
	 {
		 echo '<div class="b-pagination text-center"><ul>';
		 ...
		 for ( $i = 1; $i <= $pages; $i++ )
		 {
			 ...
			 {
				echo esc_html( $paged == $i ) ? '<li class="pag-current	"><a href="#">' . $i . '</a></li>' : '<li><a href="' . esc_url( get_pagenum_link($i) ) . '">' . esc_html( $i ) . '</a></li>';
			 }
		 }
...
function fixar_wp_get_attachment( $attachment_id ) {
...
function fixar_post_read_more(){
    $btn_name = fixar_get_option('blog_settings_readmore');
    $name = ($btn_name) ? $btn_name : esc_html__('Read More','fixar');
    return esc_attr($name);
}

function fixar_show_sidebar($type, $layout, $sidebar, $is_woo = 0) {
    ...
    $sticky_bar = fixar_get_option('blog_settings_sidebar', 'on') == 'on' && !$is_woo ? 'sticky-bar' : '';
...
function fixar_limit_words($string, $word_limit) {
    if($word_limit > 0) {
...
function fixar_get_post_terms( $args = array() ) {
	...
	$defaults = array(
		...
		'sep'        => _x( ', ', 'taxonomy terms separator', 'fixar' )
	);
...
function fixar_breadcrumbs() {
	
	/* === Options === */
	$text['home'] = wp_kses_post(__('<i class="fa fa-home fa-lg"></i>', 'fixar'));
	$text['category'] = esc_html__('Archive "%s"', 'fixar');
	$text['search'] = esc_html__('Search results for "%s"', 'fixar');
	$text['tag'] = esc_html__('Posts with tag "%s"', 'fixar');
	$text['author'] = esc_html__('%s posts', 'fixar');
	$text['404'] = esc_html__('Error 404', 'fixar');
	$text['page'] = esc_html__('Page %s', 'fixar');
	$text['cpage'] = esc_html__('Comments page %s', 'fixar');
		...
		if ( is_category() ) {
            ...
            if ( get_post_type() == 'services'  ) {
                ...
                if(!empty($cat)) {
					...
					echo esc_html__('No Categories', 'fixar');
				}
            ...
            if ( get_option('pix_admin_notice') ) {
                echo "
                    <div class='update-nag' id='pix_admin_notice'>
                        <h3 class='pix_notice_title'>" . esc_html__("Welcome to fixar", "fixar") . "</h3>
                        <p>" . esc_html__("Attention! Please install required Plugins and activate theme!", "fixar") . "</p>
                        <p>
                            <a href='" . admin_url('themes.php?page=adminpanel') . "' class='button button-primary'><i class='dashicons dashicons-nametag'></i>" . esc_html__("Read more", "fixar") . "</a>
                            <a href='#' class='button pix_hide_notice'><i class='dashicons dashicons-dismiss'></i> " . esc_html__("Hide notice", "fixar") . "</a>
                        </p>
                    </div>
                ";
            }
