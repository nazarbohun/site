<?php 
	...
	function fixar_js_vars(){
		
		$vars = '';

		$_js = apply_filters('fixar_js_vars',$vars);
	...
	function fixar_css_vars(){
		
		$css = '';
		$header_color  = fixar_get_option('header_all_color');
		$footer_color  = fixar_get_option('footer_all_color');
		if ($footer_color){
	...
	function fixar_get_theme_header($headerType = 'header1'){
	...
	function fixar_load_block($block_name){
	...
	function fixar_woo_get_page_id(){
	...
	function fixar_checkAvailableJsToPage($types){
	...
	function fixar_get_staticblock_content($blockId){
		...
		if (is_array($blockId)){
			...
			foreach($blockId as $bId){
				if ($bId == 'global'){
					$bId = fixar_get_option('footer_block');
				}
				$_block = get_post($bId);
	...
	function fixar_get_staticblock_option_array(){
