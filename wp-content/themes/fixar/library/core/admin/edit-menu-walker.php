...
class fixar_Walker_Edit_Menu extends Walker_Nav_Menu_Edit {
		...
		if (isset($matches[0][0])){
			foreach($matches[0] as $match){
				...
				if (isset($_matches[3])){
					...
					<label>
						'.esc_html__('Menu Width','fixar').'</br>
						<select name="menu-item-wide['.$elementID.']" class="css-width-field">
							<option '.$_wideDefault.' value="">'.esc_html__('Default','fixar').'</option>
							<option '.$_wideMenu.' value="menu-width">'.esc_html__('Width menu','fixar').'</option>
							<option '.$_wideFull.' value="full-width">'.esc_html__('Full width','fixar').'</option>
						</select>						
					</label></p>';
